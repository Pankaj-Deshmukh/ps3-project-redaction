...
def combine_entity_tokens(entities):
    ...
    for entry in entities:
        entity = entry["entity"]
        token = entry["token"].replace("▁", " ")  # Replace special token prefix with a space
        ...
        if entity == current_entity:
            ...
            if current_entity:
                combined[current_entity] = combined.get(current_entity, "") + "".join(current_tokens).strip() + " "
    ...
    for page_num in range(len(doc)):
        ...
        for word in words:
            ...
            if normalized_word in words_to_redact:
                ...
                page.draw_rect([x0, y0, x1, y1], color=(0, 0, 0), fill=True)  # Black fill to redact
                # Remove the underlying text to ensure it's no longer selectable or visible
                page.add_redact_annot([x0, y0, x1, y1])  # Add redaction annotation
...
def extract_text_from_pdf(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            if text:
                pages_text.append(text)  # Append the entire page text as a string
    ...
    for page_number in range(len(pdf_document)):
        ...
        for word in words_to_redact:
            ...
            for inst in text_instances:
                ...
                page.draw_rect(inst, color=(0.5, 0.5, 0.5), fill=(0.5, 0.5, 0.5))
